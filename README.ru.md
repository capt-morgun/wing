...
Лёгкий менеджер окон для macOS. Прикрепляйте окна к точным позициям, переключайтесь между открытыми окнами во всех пространствах (Spaces) с помощью удержания горячей клавиши, и перемещайте окна на другой рабочий стол — всё через клавиатурные сочетания.
...
Поскольку приложение не нотаризовано, macOS заблокирует его при первом запуске. Чтобы открыть:

1. Смонтируйте DMG и перетащите **windows.app** в папку Программы
2. Попробуйте открыть — macOS покажет *«не удаётся открыть, так как разработчик не установлен»*
3. Откройте **Системные настройки → Конфиденциальность и безопасность**
4. Прокрутите вниз и нажмите **«Всё равно открыть»**
5. Нажмите **Открыть** в диалоге подтверждения

Либо выполните команду в Терминале после копирования приложения в Программы:
```bash
