...
func switchToDesktop(_ desktopIndex: Int, from fromIndex: Int) {
    ...
    // Key codes for 1 … 9, 0 (Mission Control "Switch to Desktop N")
    let numKeyCodes = [18, 19, 20, 21, 23, 22, 26, 28, 25, 29]   // 0-indexed: pos 0 → 1, pos 9 → 0

    let desktopMod = missionControlDesktopModifier()
    let arrowMod   = missionControlArrowModifier()

    NSLog("[SpacesAPI] switchToDesktop \(fromIndex)→\(desktopIndex), visual \(fromPos)→\(toPos), \(steps) step(s)")

    // Try direct one-shot jump (requires Mission Control "Switch to Desktop N" shortcuts enabled)
    if toPos < numKeyCodes.count {
        let numKey = numKeyCodes[toPos]
        runOsascript("tell application \"System Events\" to key code \(numKey) using {\(desktopMod)}")
        Thread.sleep(forTimeInterval: 0.45)
        let afterJump = currentSpaceIndex()
        NSLog("[SpacesAPI] After direct jump (key \(numKey) + \(desktopMod)): space=\(afterJump), want=\(desktopIndex)")
    ...
    // Fallback: step-by-step arrow navigation
    ...
    for i in 0..<remainingSteps {
        lines.append("  key code \(arrowCode) using {\(arrowMod)}")
...
// MARK: - Mission Control modifier detection

/// Reads the modifier used for Mission Control "Switch to Desktop N" shortcuts
/// from ~/Library/Preferences/com.apple.symbolichotkeys.plist.
/// Returns AppleScript modifier string (e.g. "control down", "command down").
/// Falls back to "control down" (macOS default) if unreadable.
func missionControlDesktopModifier() -> String {
    let plistURL = FileManager.default.homeDirectoryForCurrentUser
    ...
    guard let data = try? Data(contentsOf: plistURL),
          ...
          let entry = hotkeys["118"] as? [String: Any],  // "Switch to Desktop 1"
    ...
    var parts: [String] = []
    if flags & 0x100000 != 0 { parts.append("command down") }
    if flags & 0x40000  != 0 { parts.append("control down") }
    if flags & 0x80000  != 0 { parts.append("option down") }
    if flags & 0x20000  != 0 { parts.append("shift down") }

    let result = parts.isEmpty ? "control down" : parts.joined(separator: ", ")
    NSLog("[SpacesAPI] Mission Control desktop modifier: %@", result)
    return result
}

/// Reads the modifier used for Mission Control "Move left/right a space" shortcuts.
/// Hotkey ID 79 = "Move left a space", 81 = "Move right a space".
func missionControlArrowModifier() -> String {
    let plistURL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Library/Preferences/com.apple.symbolichotkeys.plist")
    guard let data = try? Data(contentsOf: plistURL),
          let plist = try? PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any],
          let hotkeys = plist["AppleSymbolicHotKeys"] as? [String: Any],
          let entry = hotkeys["79"] as? [String: Any],  // "Move left a space"
          let enabled = entry["enabled"] as? Bool, enabled,
          let value = entry["value"] as? [String: Any],
          let params = value["parameters"] as? [Any],
          params.count >= 3,
    ...
    return parts.isEmpty ? "control down" : parts.joined(separator: ", ")
}
