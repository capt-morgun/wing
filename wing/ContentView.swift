        ...
        Form {
            ...
            Section(t("Window Switcher")) {
                ...
                if settings.windowSwitcherEnabled {
                    ColorPicker(t("Background color"), selection: $settings.switcherBgColor, supportsOpacity: true)
                    ColorPicker(t("Text color"), selection: $settings.switcherFontColor, supportsOpacity: true)
                    HStack {
                        Text(t("Font size"))
                        Spacer()
                        Text("\(Int(settings.switcherFontSize))pt")
                            .foregroundStyle(.secondary)
                            .monospacedDigit()
                            .frame(width: 36, alignment: .trailing)
                    }
                    Slider(value: $settings.switcherFontSize, in: 10...20, step: 1)
                    FontFamilyPicker(selection: $settings.switcherFontName)
                }
            }
            ...
            Section(t("Vim Motion")) {
                Toggle(tf("Enable %@ + H/J/K/L as arrow keys", settings.modifierKey.symbol), isOn: $settings.vimMotionEnabled)
            }
            ...
            Section(t("Hotkeys")) {
                ...
                Grid(alignment: .leading, horizontalSpacing: 20, verticalSpacing: 6) {
                    ...
                    if settings.vimMotionEnabled {
                        GridRow {
                            Text("\(mod) + H/J/K/L")
                            Text(t("Arrow keys (Vim)")).foregroundStyle(.secondary)
                        }
                        GridRow {
                            Text("").gridCellColumns(2)
                        }
...
struct FontFamilyPicker: View {
    ...
    private var displayName: String {
        selection.isEmpty ? "System Default" : selection
    }
    ...
    var body: some View {
        Picker(selection: $selection) {
            ForEach(families, id: \.self) { family in
                if family.isEmpty {
                    Text("System Default").tag(family)
                } else {
                    Text(family)
