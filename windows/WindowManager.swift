...
enum WindowManager {
    ...
    // MARK: - Move to Desktop via drag + Ctrl+Arrow
    ...
    /// macOS natively carries the dragged window to the new space — no Dock interaction needed.
    static func moveFocusedWindowToDesktop(_ desktopIndex: Int) {
        guard !moveInProgress else { return }   // ignore rapid repeated presses
